...
const cmd = [{ command: 'addWorksheet', args: { worksheetTitle: 'title' } }, { command: 'updateData', args: { data: ['1', '2', '3'], col: 2 } }];

describe('lib', () => {
  describe('validateCommands', () => {
    it('validates the command', () => {
      expect(validateCommands(JSON.stringify(cmd))).toEqual([
        { func: 'addWorksheet', kwargs: ['title'] },
        { func: 'updateData', kwargs: [['1', '2', '3'], undefined, 2] },
      ]);
    });
    ...
    it('fails the validation when command is wrong', () => {
      expect(() => validateCommands(JSON.stringify([{ command: 'wrongCommand', args: { foo: 'bar' } }]))).toThrowError(
        'Command "wrongCommand" not found - must be one of: "addWorksheet", "getWorksheet", "removeWorksheet", "updateData", "appendData", "getCellData", "getInfo"'
      );
    });
